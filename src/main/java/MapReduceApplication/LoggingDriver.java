...
public class LoggingDriver extends Configured implements Tool {  //驱动LoggingIdentityMapper类的主类
    ...
    public int run(String[] args) throws Exception {
        ...
        Job job = new Job(getConf(), "Logging job");  //Job对象指定作业执行规范，用于控制整个作业的运行，并设置作业的名称,不设置的默认情况下作业名称是JAR文件名
        ...
        job.setMapperClass(LoggingIdentityMapper.class);  //指定要使用的map类
        job.setNumReduceTasks(0);  //设置reduce任务的数量，也就是没有reduce阶段
